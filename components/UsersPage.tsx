...
import { PlusCircle, Search, Trash2, User as UserIcon, ShieldCheck, ScrollText, X, Mail, AlertTriangle, Key, Loader2, Database, Copy, Check, Info, UserPlus, Filter, Calendar } from 'lucide-react';
...
const SecurityModal: React.FC<{
    ...
    return (
         <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50" onClick={onClose}>
            <div className="bg-content-light dark:bg-content-dark rounded-xl shadow-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6 pb-2 border-b dark:border-gray-700">
                    <h3 className="text-xl font-bold flex items-center gap-2 text-gray-800 dark:text-white">
                        <Key className="text-primary" /> إدارة كلمة المرور
                    </h3>
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"><X size={20} /></button>
                </div>
                
                <div className="mb-4">
                    <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-1">المستخدم:</p>
                ...
                <div className="space-y-4 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700">
                    <h4 className="font-bold text-sm">تعيين كلمة مرور جديدة</h4>
                    <input 
                        type="text" 
                        placeholder="أدخل كلمة المرور الجديدة" 
                        value={newPassword} 
                        onChange={e => setNewPassword(e.target.value)}
                        className="w-full p-2.5 border rounded-lg dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-primary outline-none text-sm font-mono"
                    />
                    <button 
                        onClick={isSelf ? handleSelfUpdate : handleAdminDirectUpdate} 
                        disabled={!newPassword || loading}
                        className="w-full py-2.5 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:bg-gray-400 font-bold shadow-md flex justify-center items-center gap-2 mt-2"
                    >
    ...
    return (
        <div className="bg-content-light dark:bg-content-dark rounded-xl shadow-lg p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border border-gray-100 dark:border-gray-800">
            ...
            <div className="mt-4 flex flex-wrap justify-end items-center border-t border-gray-200 dark:border-gray-700 pt-3 gap-2">
                <button onClick={() => onSecurity(user)} className="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:hover:bg-yellow-900/40 transition-colors" title="إدارة كلمة المرور">
                    <Key size={18} />
                </button>
            ...
            if (userData.id) {
                ...
                if (authData.user) {
                    ...
                    if (data) {
                     ...
                     throw new Error("تم تسجيل الطلب، ولكن يجب تأكيد البريد الإلكتروني (إن كان مفعلاً).");
                }
            }
        } catch (error: any) {
